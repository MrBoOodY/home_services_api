...
class CategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $path = FilesHelper::saveFile($request->hasFile('image'), 'category_images/');
            $images[] = $path;
        ...
        $visit =  Category::create($request->all());

        if ($visit) {
            return  ResponseHelper::setResponse('success', new CategoryResource($visit));
        } else {

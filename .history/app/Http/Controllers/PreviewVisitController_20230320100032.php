...
class PreviewVisitController extends Controller
{
    ...
    {
        $Pres =  PreviewVisit::with(['visit_request:id'])->get();

        if ($Pres->isNotEmpty()) {

            return  ResponseHelper::setResponse('success', $Pres);
        } else {
    ...
    {
        $Pre = PreviewVisit::firstWhere('id', '=', $id);
        ...
        $Pre =   $Pre->update($request->all());

        if ($Pre) {
            return  ResponseHelper::setResponse('updated successfully', $Pre);
        } else {
            return ResponseHelper::setError('some thing went wrong', ResponseStatusCodes::$error_status_code, null);
        }
    }
